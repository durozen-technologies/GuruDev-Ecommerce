...
export default function CheckoutView() {
  ...
  if (success) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center py-2xl px-md text-center">
        ...
        <Link to="/shop" className="bg-primary text-on-primary font-label-md px-8 py-3 rounded-lg shadow-md hover:bg-primary-container transition-colors inline-block">
          Continue Shopping
        </Link>
      </div>
    );
  ...
  return (
    <div className="w-full max-w-[80rem] mx-auto px-md md:px-lg pt-sm pb-lg md:pt-md md:pb-xl">
      <Link to="/shop" className="flex items-center text-on-surface-variant hover:text-primary transition-colors mb-lg font-label-md w-fit">
        <ArrowLeft size={16} className="mr-2" /> Back to Shop
      </Link>

      <div className="flex flex-col lg:flex-row gap-md lg:gap-xl">
        {/* Form Section */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex-1 glass p-md md:p-lg rounded-2xl border border-outline-variant/30">
          <h2 className="font-headline-md text-on-background mb-md">Checkout</h2>
          
          <form className="space-y-md" onSubmit={(e) => { e.preventDefault(); setSuccess(true); }}>
            <section>
              <h3 className="font-label-lg text-on-surface mb-sm">Contact Information</h3>
              <input type="email" placeholder="Email Address" className="w-full bg-surface-container rounded-lg px-4 py-3 border border-outline-variant/30 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all font-body-md mb-3" required />
            </section>

            <section>
              <h3 className="font-label-lg text-on-surface mb-sm">Shipping Address</h3>
              ...
              <input type="text" placeholder="Address" className="w-full bg-surface-container rounded-lg px-4 py-3 border border-outline-variant/30 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all font-body-md mb-3" required />
            ...
            <section>
              <h3 className="font-label-lg text-on-surface mb-sm">Payment Method</h3>
            ...
            <button type="submit" className="w-full bg-primary text-on-primary font-label-lg px-8 py-4 rounded-lg hover:bg-primary-container transition-colors shadow-md mt-md uppercase tracking-wider">
              Place Order
            </button>
          </form>
        </motion.div>
        ...
        {/* Order Summary */}
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="w-full lg:w-96 glass p-md md:p-lg rounded-2xl border border-outline-variant/30 h-fit sticky top-24">
